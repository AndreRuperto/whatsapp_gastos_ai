...
# 1. Primeiro extrai dados do QR ou código de barras da imagem
for img in os.listdir("../data"):
    if img.endswith(".jpeg"):
        caminho_img = os.path.join("../data", img)
        nome = os.path.basename(img)
        print(f"\n🖼️ Testando imagem: {nome}")

        # Extração de informações da imagem (QR Code ou código de barras)
        resultado = try_all_techniques(caminho_img, nome.replace('.jpeg', ''))
        if not resultado:
            print("❌ Nenhuma informação extraída da imagem.")
            continue
        tipo = resultado.get("tipo", "").upper()
        print(f"\n{tipo}\n")
        if tipo == "QRCODE":
            print("📄 PDF sendo processado via OCR para QR Code...")
            try:
                dados = processar_qrcode_com_ocr("../data/c22e89fc-cb64-432b-bc0a-07c790b0cd81.pdf")
                descricao = gerar_descricao_para_classificacao(dados)
                print(f"📋 Descrição para classificação: {descricao}")
            except Exception as e:
                print(f"❌ Erro ao processar PDF via OCR: {e}")

        elif tipo == "PDF417" or resultado["chave"].isdigit():  # tipo de código de barras padrão
            print("📄 PDF sendo processado via PDFplumber para código de barras...")
            try:
                dados = processar_codigodebarras_com_pdfplumber("../data/Portal da Nota Fiscal Eletrônica Total.pdf")
                print_formatado(dados)
                descricao = gerar_descricao_para_classificacao(dados)
                print(f"📋 Descrição para classificação: {descricao}")
            except Exception as e:
            ...
            print("❌ Tipo de código não reconhecido. Nada será processado.")
        print("=" * 70)
        sleep(0.5)